...
local Config, Types, Players, Entities, Models, Zones, nuiData, sendData, sendDistance = Config, { {}, {}, {} }, {}, {},
    {}, {}, {}, {}, {}
local optionOrder = {}
local Selection = { index = 1, count = 0, holdStart = 0, progress = 0 }
local TargetSettings = {
    interactionColor = Config and Config.DefaultInteractionColor or '#FFD966',
    textColor = Config and Config.DefaultTextColor or '#FFFFFF'
}
...
local listSprite = {}
local kvpKey = ('%s_settings'):format(currentResourceName)
local lastPromptIcon
...
local function UpdateInteractionPrompt(distance)
    if not Config or not Config.InteractPromptText then return end
    local maxDistance = Config.InteractPromptDistance or 1.0
    distance = distance or 0.0

    if distance <= maxDistance then
        if promptActive then return end
        exports['qb-core']:DrawText(Config.InteractPromptText, {
            position = Config.InteractPromptPosition or 'bottom',
            icon = Config.InteractPromptIcon,
            iconSize = Config.InteractPromptIconSize
        })
        promptActive = true
    else
...
local function ResolveTargetEntity(key)
    if type(key) ~= 'number' then return nil end

    if DoesEntityExist(key) then
        return key
    end

    if NetworkDoesEntityExistWithNetworkId and NetToEnt then
        if NetworkDoesEntityExistWithNetworkId(key) then
            local entity = NetToEnt(key)
...
local function CalculateEntityProximity(coords)
    local closest
    for key, options in pairs(Entities) do
        if options and next(options) then
            local entity = ResolveTargetEntity(key)
            if entity then
...
local function CalculateModelProximity(coords)
    local closest
    local scanRadius = Config.InteractPromptScanRadius or 3.0

    for modelHash, options in pairs(Models) do
        if options and next(options) then
            local object = GetClosestObjectOfType(coords.x, coords.y, coords.z, scanRadius, modelHash, false, false, false)
...
local function NormalizeZoneCenter(zone, coords)
    local center = zone and zone.center
    if not center then return nil end

    local centerType = type(center)
    if centerType == 'vector2' then
        return vector3(center.x, center.y, (zone.maxZ or coords.z))
    end

    return center
end

local function CalculateZoneProximity(coords)
    local closest

    for _, zone in pairs(Zones) do
        if zone and zone.targetoptions then
            local maxDistance = zone.targetoptions.distance or Config.MaxDistance or 7.0

            if zone.isPointInside and zone:isPointInside(coords) then
                return 0.0
            end

            local center = NormalizeZoneCenter(zone, coords)
            if center then
...
local function FindClosestTargetDistance(coords)
    local closest

    local entityDistance = CalculateEntityProximity(coords)
    if entityDistance and (not closest or entityDistance < closest) then
        closest = entityDistance
    end

    local modelDistance = CalculateModelProximity(coords)
    if modelDistance and (not closest or modelDistance < closest) then
        closest = modelDistance
    end

    local zoneDistance = CalculateZoneProximity(coords)
    if zoneDistance and (not closest or zoneDistance < closest) then
        closest = zoneDistance
    end
...
local function UpdateProximityPrompt()
    if not Config or not Config.InteractPromptText then
        HideInteractionPrompt()
        return
    end
    ...
    local closest = FindClosestTargetDistance(coords)
...
local function LoadPersonalSettings()
    ...
    if ok and type(decoded) == 'table' then
        TargetSettings.interactionColor = decoded.interactionColor or TargetSettings.interactionColor
        TargetSettings.textColor = decoded.textColor or TargetSettings.textColor
    end
end

local function SavePersonalSettings()
    SetResourceKvp(kvpKey, json.encode(TargetSettings))
end

LoadPersonalSettings()

local function OpenSettingsMenu()
    if settingsOpen or IsNuiFocused() then return end
    ResetHold()
    settingsOpen = true
    ...
    SendNUIMessage({
        ...
        data = {
            interactionColor = TargetSettings.interactionColor,
            textColor = TargetSettings.textColor
        }
    })
end
...
local function ResetSelection()
    Selection.index = 1
    Selection.count = #optionOrder
    Selection.holdStart = 0
    Selection.progress = 0
end

local function SendPromptUpdate(targetIcon)
    local optionsPayload = {}
    for i = 1, #optionOrder do
        local slot = optionOrder[i]
        local data = nuiData[slot]
        if data then
    ...
    Selection.count = #optionsPayload
    if Selection.count == 0 then
        Selection.index = 1
        Selection.progress = 0
    elseif Selection.index > Selection.count then
    ...
    if targetIcon then
        lastPromptIcon = targetIcon
    end
    local iconToSend = targetIcon or lastPromptIcon

    SendNUIMessage({
        response = 'promptUpdate',
        data = {
            icon = iconToSend,
            options = optionsPayload,
            selected = Selection.index,
            keyLabel = Config.InteractKeyLabel,
...
local function UpdateHoldProgress(progress)
    Selection.progress = progress
    SendNUIMessage({ response = 'holdProgress', data = progress })
end

ResetHold = function()
    if Selection.progress ~= 0 then
        UpdateHoldProgress(0)
    end
    Selection.holdStart = 0
end

local function ScreenDataValid(value)
    return value and value == value and value ~= math_huge and value ~= -math_huge
end

local function VectorValid(vec)
    if not vec then return false end
    return ScreenDataValid(vec.x) and ScreenDataValid(vec.y) and ScreenDataValid(vec.z)
end

local function EnsureScreenData()
    if not ScreenDataValid(screen.ratio) then
        screen.ratio = GetAspectRatio(true)
    end
    if not ScreenDataValid(screen.fov) then
        screen.fov = GetFinalRenderedCamFov()
    end
end

local function GetSelectedSlot()
    return optionOrder[Selection.index]
end

local function CycleSelection(direction)
    if Selection.count <= 1 then return end
    Selection.index = Selection.index + direction
    if Selection.index > Selection.count then
        Selection.index = 1
    elseif Selection.index < 1 then
        Selection.index = Selection.count
    end
    ...
    local icon = slot and nuiData[slot] and nuiData[slot].targeticon or nil
    SendPromptUpdate(icon)
end

local function MenuControlPressed()
    return Config.MenuControlKey and Config.MenuControlKey >= 0 and IsDisabledControlPressed(0, Config.MenuControlKey)
end

local function HandlePromptInputs()
    if Selection.count == 0 then return false end

    DisableControlAction(0, Config.InteractControl, true)
    DisableControlAction(0, Config.ScrollUpControl, true)
    DisableControlAction(0, Config.ScrollDownControl, true)
    DisableControlAction(0, Config.SettingsOpenControl, true)

    if settingsOpen then
        return false
    end

    if IsDisabledControlPressed(0, Config.SettingsModifierControl) and IsDisabledControlJustPressed(0, Config.SettingsOpenControl) then
        OpenSettingsMenu()
        return false
    end

    if IsDisabledControlJustPressed(0, Config.ScrollUpControl) then
        CycleSelection(-1)
    elseif IsDisabledControlJustPressed(0, Config.ScrollDownControl) then
        CycleSelection(1)
    end

    if IsDisabledControlPressed(0, Config.InteractControl) then
        if Selection.holdStart == 0 then
            Selection.holdStart = GetGameTimer()
        end
        local elapsed = GetGameTimer() - Selection.holdStart
        local progress = math_min(elapsed / Config.HoldDuration, 1.0)
        UpdateHoldProgress(progress)
        if progress >= 1.0 then
            local slot = GetSelectedSlot()
            if slot then
                ActivateOptionBySlot(slot)
                return true
            end
        end
    elseif Selection.holdStart ~= 0 or Selection.progress ~= 0 then
        ResetHold()
    end

    if IsDisabledControlJustReleased(0, Config.InteractControl) then
        ResetHold()
    end
...
ActivateOptionBySlot = function(slot)
    local data = sendData[slot]
    if not data then return end
    ResetHold()
    targetActive, success, hasFocus = false, false, false
    table_wipe(nuiData)
    table_wipe(sendDistance)
    SendNUIMessage({ response = 'promptClose' })
    table_wipe(optionOrder)
    table_wipe(listSprite)
    lastPromptIcon = nil
    CreateThread(function()
        ...
        if data.action then
            data.action(data.entity, data.coords)
        elseif data.event then
            if data.serverEvent or data.type == 'server' then
                TriggerServerEvent(data.event, data)
            else
                TriggerEvent(data.event, data)
            end
        elseif data.command then
            if data.commandIsClient then
                ExecuteCommand(data.command)
            else
                TriggerServerEvent('qb-target:server:runCommand', data.command)
            end
        elseif data.canInteract then
            data.canInteract(data.entity, data.distance, data.data)
        end
    end)
end
	...
	CreateThread(function()
		...
		while targetActive do
			...
			for _, zone in pairs(listSprite) do
				...
				DrawSprite('shared', 'emptydot_32', 0, 0, 0.004, 0.004, 0, r, g, b, a)
				ClearDrawOrigin()
			end
			Wait(sleep)
		end
...
local function RaycastCamera(flag, playerCoords)
    ...
    if not VectorValid(rayPos) or not VectorValid(rayDir) then
        return playerCoords, 0.0, nil, 0
    end
...
local function SetupOptions(datatable, entity, distance, isZone)
    ...
    for _, data in pairs(datatable) do
        if CheckOptions(data, entity, distance) then
            ...
            optionOrder[#optionOrder + 1] = slot
            if not isZone then
...
local function CheckEntity(flag, datatable, entity, distance)
	...
	SendPromptUpdate(nuiData[slot].targeticon)
	UpdateInteractionPrompt(distance)
	DrawOutlineEntity(entity, true)
	while targetActive and success do
		local _, dist, entity2 = RaycastCamera(flag)
		UpdateInteractionPrompt(dist)
		if entity ~= entity2 then
			LeftTarget()
			DrawOutlineEntity(entity, false)
			break
		elseif not hasFocus and MenuControlPressed() then
			EnableNUI(nuiData)
			DrawOutlineEntity(entity, false)
		elseif HandlePromptInputs() then
			break
		else
		...
		if distance <= Config.MaxDistance then
			if entityType > 0 then
				...
				elseif entityType == 2 then
					...
					local modelData = Models[GetEntityModel(entity)]
					if modelData then CheckEntity(flag, modelData, entity, distance) end
...
RegisterNUICallback('applySettings', function(data, cb)
    if type(data) == 'table' then
        if data.interactionColor then TargetSettings.interactionColor = data.interactionColor end
        if data.textColor then TargetSettings.textColor = data.textColor end
        SavePersonalSettings()
        if targetActive then
            SendPromptUpdate(lastPromptIcon)
        end
        SetNuiFocus(false, false)
        SetNuiFocusKeepInput(false)
        settingsOpen = false
        SendNUIMessage({ response = 'settingsClosed' })
    end
    cb('ok')
end)
...
if Config.PersonalizeCommand and Config.PersonalizeCommand ~= '' then
    RegisterCommand(Config.PersonalizeCommand, function()
        OpenSettingsMenu()
    end, false)
    TriggerEvent('chat:removeSuggestion', ('/%s'):format(Config.PersonalizeCommand))
end
